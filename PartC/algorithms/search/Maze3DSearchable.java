...
public class Maze3DSearchable implements Searchable 
{
	...
	public State getInitialState() 
	{
		...
		else if(start.getZ() == this.maze.getzAxis()-1)
		{
	...
	public State getGoalState() 
	{
		...
		else if(goal.getZ() == this.maze.getzAxis()-1)
		{
	...
	*/
	...
	//Getting all the free positions 
			public ArrayList<Position> getFreePositions(Position p)
			{
				...
				if (this.maze.canMove(p, upFloor))
				{
					moves.add(upFloor);
		 		}
				if (this.maze.canMove(p, downFloor))
				{
					moves.add(downFloor);
		 		}
				if (this.maze.canMove(p, forward))
				{
					moves.add(forward);
		 		}
				if (this.maze.canMove(p, backward))
				{
					moves.add(backward);
		 		}
				if (this.maze.canMove(p, right))
				{
					moves.add(right);
		 		}
				if (this.maze.canMove(p, left))
				{
					moves.add(left);
		 		}
				return moves;	
