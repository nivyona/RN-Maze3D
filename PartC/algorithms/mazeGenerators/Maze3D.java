...
public class Maze3D  
{
	...
	public boolean createRandomPath() 
	{
			...
			int choose = rand.nextInt(6);	
				...
				{	
				case 0: // X axis
					if(this.breakUpFloor())
					{
						return true;
					}
					break;
				case 1:	// Y axis
					if(this.breakForward())
					{
						return true;
					}
					break;
				case 2:	// Z axis
					...
					break;
				case 3: // X axis
					if(this.breakDownFloor())
					{
						return true;
					}
					break;
				case 4:	// Y axis
					if(this.breakBackward())
					{
						return true;
					}
					break;
	...
	{
		Position p1 = new Position(this.getCurrent());
		Position p2 = new Position(this.getCurrent());
		p1.setY(p1.getY()+1);
		p2.setY(p2.getY()+2);
		
		if(this.cellFree(p1) && this.cellFree(p2))
		{
			this.setCurrent(p2);
			return true;
		}
	...
	{
		byte[] res = new byte[12+(this.getxAxis()*this.getyAxis()*this.getzAxis())];
		...
		res[9] = (byte)this.getCurrent().getX();
		res[10] = (byte)this.getCurrent().getY();
		res[11] = (byte)this.getCurrent().getZ();
		int count = 12;
		for (int i = 0; i < this.getxAxis(); ++i)
		{
			for (int j = 0; j < this.getyAxis(); ++j)
	...
	public Maze3D(byte[] mazeBytes)
	{
		...
		this.setCurrent(new Position(mazeBytes[9],mazeBytes[10],mazeBytes[11]));
		int count = 12;
		for (int i = 0; i < this.getxAxis(); ++i)
		{
			for (int j = 0; j < this.getyAxis(); ++j)
	...
	public void resetPosition()
	{
		...
		Position p = this.getStartPosition();
		if(p == null)
			return;
		if (p.getY()==0)
		{
	...
	public boolean canMove(Position start, Position goal)
	{
		Position between = new Position(goal);
		if(start.getY() != goal.getY())
		{
			between.setY(start.getY() - (start.getY()-goal.getY())/2);
		}
		else if(start.getZ() != goal.getZ())
		{
			between.setZ(start.getZ() - (start.getZ()-goal.getZ())/2);
		}
		if (this.positionExists(goal) &&  this.getMaze()[goal.getX()][goal.getY()][goal.getZ()] == 0 &&
				this.positionExists(between) && this.getMaze()[between.getX()][between.getY()][between.getZ()] == 0)
		{
			return true;
		}
		return false;
